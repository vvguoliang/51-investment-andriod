 ...
 * ******┗┻┛  ┗┻┛
 */
class LastPriceService : Service() {
    ...
    inner class Binder : android.os.Binder() {
        val service: LastPriceService
            get() = this@LastPriceService
    }
    ...
    private fun getPushThread() {
        Thread(Runnable {
            while (pushthread) {
                try {
                    Thread.sleep(1000)
                    HttpALl.get().setLastPrice(qtecode, mHandler, this)
    ...
    fun getConnet(mContext: Context, qtecode: String) {
        try {
            val intent = Intent(mContext, LastPriceService::class.java)
