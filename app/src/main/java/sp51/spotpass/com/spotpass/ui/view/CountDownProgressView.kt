...
/**
 * @Time : 2018/5/14 no 10:48
 * @USER : vvguoliang
 * @File : CountDownProgressView.java
 ...
 * ******┗┻┛  ┗┻┛
 */
class CountDownProgressView : TextView {
    //TODO 圆实心的颜色
    private var circSolidColor: Int = 0

    //TODO 圆边框的颜色
    private var circFrameColor: Int = 0

    //TODO 圆边框的宽度
    private val circFrameWidth = 4

    //TODO 圆的半径
    private var circRadius: Int = 0

    //TODO 进度条的颜色
    private var progressColor: Int = 0

    //TODO 进度条的宽度
    private val progressWidth = 4
    ...
    //TODO 进度倒计时时间
    private var timeMillis: Long = 800
    ...
    private val progressChangeTask = object : Runnable {
        override fun run() {
            ...
            when (mProgressType) {
                CountDownProgressView.ProgressType.COUNT -> progress += 1
                CountDownProgressView.ProgressType.COUNT_BACK -> progress -= 1
            }
    ...
    constructor(context: Context, attrs: AttributeSet) : super(context, attrs) {
        init()
        val typedArray = getContext().obtainStyledAttributes(attrs, R.styleable.CountDownProgress)
        if (typedArray != null) {
            //TODO 圆实心的颜色属性
            ...
            if (typedArray.hasValue(R.styleable.CountDownProgress_textColor)) {
                //有这个属性索引 获取用户设置的颜色
                Companion.textColor = typedArray.getColor(R.styleable.CountDownProgress_textColor, 0)
            } else {
                //没有这个属性索引 默认是白色
                Companion.textColor = typedArray.getColor(R.styleable.CountDownProgress_textColor, Color.parseColor("#ffffff"))
            }
    ...
    protected override fun onDraw(canvas: Canvas) {
        ...
        canvas.drawCircle(mBounds!!.centerX().toFloat(), mBounds!!.centerY().toFloat(), this.circRadius.toFloat(), mPaint)


        //TODO 画外边框(空心圆,即园边框)
        mPaint!!.setAntiAlias(true)//设置抗锯齿
        mPaint!!.setStyle(Paint.Style.STROKE)//空心style
        mPaint!!.setStrokeWidth(circFrameWidth.toFloat())//设置空心线宽度
        mPaint!!.setColor(circFrameColor)
        canvas.drawCircle(mBounds!!.centerX().toFloat(), mBounds!!.centerY().toFloat(), (circRadius - circFrameWidth).toFloat(), mPaint)

        //TODO 画文字
        val text_paint = paint //注意：如果是继承的view，这里是没有这个getPaint()方法的。大家可以看到它是Textview包下的方法
        text_paint.color = Companion.textColor
        text_paint.isAntiAlias = true
        text_paint.textAlign = Paint.Align.CENTER
        val textY = mCenterY - (text_paint.descent() + text_paint.ascent()) / 2
        canvas.drawText(mText, mCenterX.toFloat(), textY, text_paint)


        //TODO 画进度条
        mPaint!!.setColor(progressColor)
        mPaint!!.setStyle(Paint.Style.STROKE)
        mPaint!!.setStrokeWidth(progressWidth.toFloat())
        mPaint!!.setStrokeCap(Paint.Cap.ROUND)  //当画笔样式为STROKE或FILL_OR_STROKE时，设置笔刷的图形样式，如圆形样式  Cap.ROUND,或方形样式Cap.SQUARE
        mArcRectF!!.set((mBounds!!.left + progressWidth).toFloat(), (mBounds!!.top + progressWidth).toFloat(), (mBounds!!.right - progressWidth).toFloat(), (mBounds!!.bottom - progressWidth).toFloat())
        canvas.drawArc(mArcRectF, (-90).toFloat(), (360 * progress / 100).toFloat(), false, mPaint) //这里的-90.是方向，大家可以改成0，90，180，-90等就可以看到效果区别
    ...
    private fun resetProgress() {
        when (mProgressType) {
            CountDownProgressView.ProgressType.COUNT -> progress = 0
            CountDownProgressView.ProgressType.COUNT_BACK -> progress = 100
        }
    }
     ...
     */
    fun setProgressListener(mProgressListener: OnProgressListener) {
