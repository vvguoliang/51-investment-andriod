...
import sp51.spotpass.com.spotpass.ui.kchartlib.entity.IMinuteLine
...
/**
 * @Time : 2018/5/10 no 18:16
 * @USER : vvguoliang
 * @File : MinuteChartView.java
...
class MinuteChartView : View, GestureDetector.OnGestureListener {
    ...
    private fun initAttrs(attrs: AttributeSet) {
        val array = context.obtainStyledAttributes(attrs, R.styleable.MinuteChartView)
        if (array != null) {
            setBackgroundColor(array.getColor(R.styleable.MinuteChartView_GridPaintColole, getColor(R.color.chart_tab_background)))
            setGridPaint(array.getColor(R.styleable.MinuteChartView_BackgroundColor, getColor(R.color.chart_tab_background)))
        }
    }
     ...
     * 设置背景颜色
     */
    private fun setGridPaint(color: Int) {
    ...
    private fun getX(position: Int): Float {
        val date = mPoints[position].time
        return if (mSecondStartTime != null && date!!.getTime() >= mSecondStartTime!!.time) {
     ...
     */
    fun initData(data: Collection<IMinuteLine>,
                 startTime: Date,
                 endTime: Date,
                 yesClosePrice: Float) {
    ...
    fun notifyChanged() {
        ...
        for (i in mPoints.indices) {
            val point = mPoints[i]
            mValueMax = Math.max(mValueMax, point.price)
            mValueMin = Math.min(mValueMin, point.price)
            mVolumeMax = Math.max(mVolumeMax, point.volume)
        }
        //最大值和开始值的差值
        val offsetValueMax = mValueMax - mValueStart
        val offsetValueMin = mValueStart - mValueMin
        //以开始的点为中点值   上下间隙多出20%
        val offset = (if (offsetValueMax > offsetValueMin) offsetValueMax else offsetValueMin) * 1.2f
        //坐标轴高度以开始的点对称
    ...
    override fun onDraw(canvas: Canvas) {
        ...
        if (mPoints.size > 0) {
            ...
            for (i in mPoints.indices) {
                ...
                val curX = getX(i)
                canvas.drawLine(lastX, getY(lastPoint.price), curX, getY(curPoint.price), mPricePaint)
                canvas.drawLine(lastX, getY(lastPoint.avg), curX, getY(curPoint.avg), mAvgPaint)
                //成交量
                val volumePaint = if (i == 0 && curPoint.price <= mValueStart || curPoint.price <= lastPoint.price) mVolumePaintGreen else mVolumePaintRed
                canvas.drawLine(curX, getVolumeY(0f), curX, getVolumeY(curPoint.volume), volumePaint)
                lastPoint = curPoint
                lastX = curX
            }
        }
        ...
        if (isLongPress) {
            ...
            canvas.drawLine(0f, getY(point.price), mWidth.toFloat(), getY(point.price), mTextPaint)
            //画指示线的时间
            var text = DateUtil.shortTimeFormat.format(point.time)
            x -= mTextPaint.measureText(text) / 2
            if (x < 0) {
                x = 0f
            }
            ...
            //下方时间
            canvas.drawRect(x, mHeight + mVolumeHeight - baseLine + textHeight, x + mTextPaint.measureText(text), mVolumeHeight.toFloat() + mHeight.toFloat() + baseLine, mBackgroundPaint)
            canvas.drawText(text, x, mHeight.toFloat() + mVolumeHeight.toFloat() + baseLine, mTextPaint)

            val r = textHeight / 2
            val y = getY(point.price)
            //左方值
            text = floatToString(point.price)
            canvas.drawRect(0f, y - r, mTextPaint.measureText(text), y + r, mBackgroundPaint)
            canvas.drawText(text, 0f, fixTextY(y), mTextPaint)
            //右方值
            text = floatToString((point.price - mValueStart) * 100f / mValueStart) + "%"
            canvas.drawRect(mWidth - mTextPaint.measureText(text), y - r, mWidth.toFloat(), y + r, mBackgroundPaint)
...
//            var text = "成交价:" + floatToString(point.price) + " "
//            var x = 0f
//            canvas.drawText(text, x, y, mPricePaint)
//            x += mPricePaint.measureText(text)
//            text = "均价:" + floatToString(point.avg) + " "
//            canvas.drawText(text, x, y, mAvgPaint)
//            //成交量
//            text = "VOL:" + mVolumeFormatter!!.format(point.volume)
...
//        }
    ...
    private fun drawGird(canvas: Canvas) {
        ...
        canvas.drawLine(0f, rowSpace * mGridRows / 2, mWidth.toFloat(), rowSpace * mGridRows / 2, mGridPaint)

        canvas.drawLine(0f, (mHeight + mVolumeHeight).toFloat(), mWidth.toFloat(), (mHeight + mVolumeHeight).toFloat(), mGridPaint)
        //纵向的grid
        val columnSpace = (mWidth / GridColumns).toFloat()
        for (i in 0..GridColumns) {
            canvas.drawLine(columnSpace * i, 0f, columnSpace * i, (mHeight + mVolumeHeight).toFloat(), mGridPaint)
        }
     ...
     * 解决text居中的问题
     */
    private fun fixTextY(y: Float): Float {
    ...
    private fun drawText(canvas: Canvas) {
        ...
        canvas.drawText(DateUtil.shortTimeFormat.format(mFirstStartTime), 0f, y, mTextPaint)
