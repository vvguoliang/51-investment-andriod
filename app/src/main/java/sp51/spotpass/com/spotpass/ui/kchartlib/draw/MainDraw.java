...
import sp51.spotpass.com.spotpass.ui.kchartlib.entity.ICandle;
import sp51.spotpass.com.spotpass.ui.kchartlib.entity.IKLine;
import sp51.spotpass.com.spotpass.ui.kchartlib.formatter.ValueFormatter;
...
public class MainDraw extends BaseChartDraw<ICandle> {
    ...
    public void drawValues(@NonNull Canvas canvas, int start, int stop) {
        int x = 0;
        int y = 0;
        ICandle point = (IKLine) this.getMKChartView().getItem(this.getMKChartView().isLongPress() ? this.getMKChartView().getSelectedIndex() : this.getMKChartView().getStopIndex());

        new CanvasUtils().drawTexts(canvas, x, y, XAlign.LEFT, YAlign.BOTTOM,
                new Pair<>(ma5Paint, "MA5:" + this.getMKChartView().formatValue(point.getMA5Price()) + " "),
                new Pair<>(ma10Paint, "MA10:" + this.getMKChartView().formatValue(point.getMA10Price()) + " "),
                new Pair<>(ma20Paint, "MA20:" + this.getMKChartView().formatValue(point.getMA20Price()) + " "));

        if (this.getMKChartView().isLongPress()) {
            drawSelector(this.getMKChartView(), canvas);
        }
    }
    ...
    private void drawCandle(Canvas canvas, float x, float high, float low, float open, float close) {
        ...
        if (open > close) {
            ...
            if (mCandleSolid) {
                ...
                canvas.drawLine(x + r - lineR, open, x + r - lineR, close, mRedPaint);
                mRedPaint.setStrokeWidth(mCandleLineWidth * this.getMKChartView().getScaleX());
    ...
    private void drawSelector(BaseKChartView view, Canvas canvas) {
        ...
        float padding = ViewUtil.INSTANCE.Dp2Px(mContext, 5);
        float margin = ViewUtil.INSTANCE.Dp2Px(mContext, 5);
        float width = 0;
        float left;
        ...
        strings.add(view.formatDateTime(view.getAdapter().getDate(index), 1));
