...
/**
 * @Time : 2018/5/10 no 18:54
 * @USER : vvguoliang
 * @File : BaseKChartView.java
...
import sp51.spotpass.com.spotpass.ui.kchartlib.formatter.TimeFormatter;
...
//        mKChartTabView.setOnTabSelectListener(type -> setChildDraw(type));
...
//                String text = formatDateTime(mAdapter.getDate(index), 1);
...
//
        ...
        if (translateX >= startX && translateX <= stopX) {
            canvas.drawText(formatDateTime(getAdapter().getDate(mStartIndex), 2), 0, y, mTextPaint);
        }
        translateX = xToTranslateX(mWidth);
        if (translateX >= startX && translateX <= stopX) {
            String text = formatDateTime(getAdapter().getDate(mStopIndex), 2);
...
//        mKChartTabView.addTab(name);
     ...
     * @param date
     */
    public String formatDateTime(Date date, int intype) {
        if (getDateTimeFormatter() == null) {
            setDateTimeFormatter(new TimeFormatter());
        }
        return getDateTimeFormatter().format(date, intype);
    }
