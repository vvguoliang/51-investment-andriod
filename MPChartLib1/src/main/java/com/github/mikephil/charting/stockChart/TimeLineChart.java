...
public class TimeLineChart extends LineChart {
    ...
    private BarTomMarkerView markerTom;
    ...
    /*返回转型后的左右轴*/
    public void setMarker(LeftMarkerView markerLeft, TimeRightMarkerView markerRight, BarTomMarkerView markerTom, KTimeData kTimeData) {
        this.myMarkerViewLeft = markerLeft;
        this.markerTom = markerTom;
    ...
    protected void drawMarkers(Canvas canvas) {
        ...
        for (int i = 0; i < mIndicesToHighlight.length; i++) {
            ...
            String date = DataTimeUtil.secToTime(kTimeData.getDatas().get((int) mIndicesToHighlight[i].getX()).getTimeMills());//分时图显示的数据
            ...
            markerTom.setData(date);
            markerTom.refreshContent(e, mIndicesToHighlight[i]);
            markerTom.measure(MeasureSpec.makeMeasureSpec(0, MeasureSpec.UNSPECIFIED), MeasureSpec.makeMeasureSpec(0, MeasureSpec.UNSPECIFIED));
            markerTom.layout(0, 0, markerTom.getMeasuredWidth(), markerTom.getMeasuredHeight());

            int width = markerTom.getWidth() / 2;
            if (mViewPortHandler.contentRight() - pos[0] <= width) {
                markerTom.draw(canvas, mViewPortHandler.contentRight() - markerTom.getWidth() / 2, mViewPortHandler.contentBottom() + markerTom.getHeight());//-markerBottom.getHeight()   CommonUtil.dip2px(getContext(),65.8f)
            } else if (pos[0] - mViewPortHandler.contentLeft() <= width) {
                markerTom.draw(canvas, mViewPortHandler.contentLeft() + markerTom.getWidth() / 2, mViewPortHandler.contentBottom() + markerTom.getHeight());
            } else {
                markerTom.draw(canvas, pos[0], mViewPortHandler.contentBottom() + markerTom.getHeight());
            }
